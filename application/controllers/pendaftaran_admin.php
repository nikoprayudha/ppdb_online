	...
	public function __construct(){
		...
		$data2=$this->member_model->getMember($id);
			...
			else
		{
			...
			$this->member_model->UpdateById($id);
			$this->load->view('tambah_member_sukses',$data);
		}
...
// 	}
	...
	{
		$this->load->model('member_model');
		$data = $this->member_model->cekdeleteById($id);
		...
		if ($data == NULL) {
			...
			redirect('member',$data,'refresh');
			// $this->load->view('hapus_member_view',$data);	

		} else {
			echo'<script type="text/javascript">alert("Member Tidak Bisa Dihapus!")</script>';
			redirect('member', 'refresh');
		}
